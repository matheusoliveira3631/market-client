    ...
    <div id='fg'>
      <div id='login-ctn' class='rounded d-flex flex-column'>
        <h5 class='d-flex flex-column' id='title'>
          ...
          <p id='password-error' v-if="passwordError">{{errorData}}</p> 
        </h5> 
...
export default {
    ...
    methods:{
      ...
      submit:async function(obj){
        let url=''
        let redirectUrl=''
        obj.id=='employeeButton' ? [url,redirectUrl] = [`${apiUrl}/login`,'clerk'] : [url,redirectUrl] = [`${apiUrl}/login/admin`,'admin']
        if (this.employeeId && this.password){
          ...
          axios.post(url ,{
            ...
            const errorList={
              500:'I.D Inválido',
              401:'Senha incorreta',
              403:'Acesso de administrador proibido'
            }
            setTimeout(()=>{
              this.passwordError=true
              this.errorData=errorList[err.response.status]
            },500)    
            setTimeout(()=>{
